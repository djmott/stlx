/**
 * @file xstring.hpp
 * @brief Extended string utilities for C++23
 *
 * This file provides extended string functionality including advanced string
 * manipulation, formatting, splitting, trimming, and case conversion utilities.
 * It extends std::basic_string with additional methods for common string
 * operations.
 *
...
#ifndef STLX_XSTRING_HPP
#define STLX_XSTRING_HPP
...
public:
  ...
  xstring(_ArgsT &&...oArgs) : _super_t(std::forward<_ArgsT>(oArgs)...) {}

  bool ends_with(const stlx::string &suffix) const {
    if (suffix.empty())
      return true;
    if (_super_t::size() < suffix.size())
      return false;

    auto sDest = _super_t::rbegin();
    auto sSrc = suffix.rbegin();
    for (; suffix.rend() != sSrc && sDest != _super_t::rend();
         ++sSrc, ++sDest) {
      if (*sSrc != *sDest)
        return false;
    }
  ...
  template <typename T> static xstring to_string(const T &val) {
  ...
  template <size_t N> static xstring to_string(const char (&val)[N]) {
  ...
  static xstring from_resource(uint32_t resid) {
    ...
    for (;;) {
      auto iRet =
          LoadString(GetModuleHandle(nullptr), resid, &sRet[0], sRet.size());
      if (!iRet)
        return xstring();
  ...
  // replaces all occurrences of the characters in the oItems list with a
  // specified character
  ...
  xstring &remove(const std::initializer_list<_ChT> &chars) {
    auto it =
        std::remove_if(_super_t::begin(), _super_t::end(), [&chars](_ChT ch) {
  ...
  size_type find_first_of(const std::initializer_list<_ChT> &delimiters,
  ...
  size_type find_first_of(const std::function<bool(_ChT)> &VisitorFN,
                          size_type pos = 0) const {
    for (size_type i = pos; i < _super_t::size(); ++i) {
      if (VisitorFN(_super_t::at(i)))
        return i;
    }
    return _super_t::npos;
  }

  size_type find_last_of(const std::initializer_list<_ChT> &delimiters,
                         size_type pos = _super_t::npos) const {
    size_type iRet = _super_t::npos;
    for (const _ChT ch : delimiters) {
  ...
  std::vector<xstring<_ChT>>
  split(const std::initializer_list<_ChT> &delimiters,
        bool trimEmpty = false) const {
    ...
    forever {
      ...
      if (pos == _my_t::npos) {
        ...
        if (pos != lastPos || !trimEmpty) {
          oRet.push_back(
              value_type(_super_t::data() + lastPos, (pos - lastPos)));
        }
        break;
  ...
  std::vector<xstring<_ChT>> split(const xstring<_ChT> &delim,
                                   bool trimEmpty = false) const {
    using container_t = std::vector<xstring<_ChT>>;
    container_t oRet;
    using _my_t = xstring<_ChT>;
  ...
  std::vector<xstring<_ChT>>
  split(const std::function<bool(_ChT)> &VisitorFN) const {
    using container_t = std::vector<xstring<_ChT>>;
    using value_type = typename container_t::value_type;
    container_t oRet;
    using _my_t = xstring<_ChT>;
    ...
    if (iLast < _my_t::size()) {
      oRet.push_back(
          value_type(_super_t::data() + iLast, _my_t::size() - iLast));
    }
...
struct iterator_traits<stlx::xstring<_ChT>>
    : std::iterator_traits<std::basic_string<_ChT>> {};
} // namespace std

#endif // STLX_XSTRING_HPP