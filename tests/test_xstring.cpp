/**
 * @file test_xstring.cpp
 * @brief Unit tests for STLX extended string utilities
 *
 * This file contains comprehensive unit tests for the STLX xstring class
 * including string manipulation, formatting, splitting, trimming, and case
 * conversion.
 *
...
TEST_F(XStringTest, Construction) {
  ...
  EXPECT_EQ(s5, "Hello");
  // Note: s2 is now in moved-from state, accessing it is undefined behavior
  // EXPECT_TRUE(s2.empty()); // This line causes undefined behavior
}
...
TEST_F(XStringTest, Split) {
  ...
  auto result6 =
      s5.split(std::function<bool(char)>([](char c) { return c == ' '; }));
...
TEST_F(XStringTest, Performance) {
  ...
  EXPECT_EQ(
      s, "Hello"); // Should be back to original after even number of reverses
}
