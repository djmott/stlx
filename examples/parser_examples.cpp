...
namespace expression_example {
    ...
    using term = or_<std::string::const_iterator, number, and_<std::string::const_iterator, lparen, number, rparen>>;
    using expression = and_<std::string::const_iterator, term, zero_or_more_<std::string::const_iterator, and_<std::string::const_iterator, plus, term>>>; // Simplified
...
namespace statement_example {
    ...
    using number = one_or_more_<std::string::const_iterator, digit>;
    using assignment = and_<std::string::const_iterator, identifier, equals, number, semicolon>;

    void demonstrate_statement_parsing() {
        std::cout << "\n=== Statement Parsing Example ===" << std::endl;
        
        std::vector<std::string> test_statements = {
            "x=42;",
            "y=123;",
            "invalid",
            "a=;"
...
namespace json_example {
    ...
    using string_char = or_<std::string::const_iterator, letter, digit, character<std::string::const_iterator, ' '>>;
    using json_string = and_<std::string::const_iterator, quote, zero_or_more_<std::string::const_iterator, string_char>, quote>;
    ...
    void demonstrate_json_parsing() {
        ...
        std::vector<std::string> test_json = {
            "\"hello\"",
            "123",
            "\"test value\"",
            "invalid"
...
namespace regex_example {
    ...
    void demonstrate_regex_parsing() {
        ...
        std::vector<std::string> test_phones = {
            "(555) 123-4567",
            "invalid-phone",
            "(999) 888-7777"
        };
        ...
        for (const auto& email : test_emails) {
            ...
            email_regex parser;
            bool success = parser.parse(ctx, begin, end);
            std::cout << "  '" << email << "': " << (success ? "MATCH" : "NO MATCH") << std::endl;
        }
        ...
        for (const auto& phone : test_phones) {
            ...
            phone_regex parser;
            bool success = parser.parse(ctx, begin, end);
            std::cout << "  '" << phone << "': " << (success ? "MATCH" : "NO MATCH") << std::endl;
        }
        ...
        for (const auto& ip : test_ips) {
            ...
            ip_regex parser;
            bool success = parser.parse(ctx, begin, end);
            std::cout << "  '" << ip << "': " << (success ? "MATCH" : "NO MATCH") << std::endl;
        }
    }
}
...
namespace combinator_example {
    ...
    using identifier_char = or_<std::string::const_iterator, letter, digit, underscore>;
    using identifier = and_<std::string::const_iterator, letter, zero_or_more_<std::string::const_iterator, identifier_char>>;
